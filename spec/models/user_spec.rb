...
RSpec.describe User, type: :model do
  context 'test creation of user' do
    ...
    it 'return false if is created invalid user with duplicate name' do
      ...
      u1.save
      u2 = User.new({ email: 'test1@as.com', name: 'Test1', password: '123456', password_confirmation: '123456' })
    ...
    it 'create invalid user with duplicate email' do
      u1 = User.new({ email: 'test1@s.com', name: 'Test1', password: '123456', password_confirmation: '123456' })
      u1.save
      u2 = User.new({ email: 'test1@s.com', name: 'Test1a', password: '123456', password_confirmation: '123456' })
      expect(u2.valid?).to eq(false)
    end

    it 'return false if created invalid user with missing email' do
      u1 = User.new({ name: 'Test1', password: '123456', password_confirmation: '123456' })
      u1.save
      expect(u1.valid?).to eq(false)
    end

    it 'return false if created invalid user with missing password' do
      u1 = User.new({ email: 'test1@s.com', name: 'Test1', password_confirmation: '123456' })
      u1.save
      expect(u1.valid?).to eq(false)
    end
  end
